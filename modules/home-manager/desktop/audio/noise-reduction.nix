...
let
  cfg = config.homeManagerModules.desktop.audio.noiseReduction;
in
{
  options.homeManagerModules.desktop.audio.noiseReduction = {
    ...
    advanced = {
      ...
      aggressiveness = mkOption {
        type = types.float;
        default = 0.8;
        description = "Noise reduction aggressiveness (0.0-1.0)";
      };
    };
  ...
  config = mkIf cfg.enable {
      ...
      context.modules = [
        {
          ...
          args = {
              ...
              nodes = [
                {
                  ...
                  control = {
                    ...
                    ${optionalString cfg.advanced.enableVad ''"Enable VAD" = 1''}
                    "Noise Reduction" = ${toString cfg.advanced.aggressiveness}
                  }
                }
              ]
            }
            capture.props = {
              ...
              node.passive = true
              audio.rate = 48000
              audio.channels = ${if cfg.advanced.channels == "stereo" then "2" else "1"}
    ...
    home.sessionVariables = {
      PIPEWIRE_NOISE_REDUCTION = if cfg.enable then "1" else "0";
    };
  };
}
